<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(n, index) in addressInfo" :key="n.id">
        <span class="username " :class="{selected:n.isDefault==1} ">{{ n.consignee }}</span>
        <p @click="changeDefault(n,addressInfo )">
          <span class="s1">{{ n.fullAddress }}</span>
          <span class="s2">{{ n.phoneNum }}</span>
          <span class="s3" v-show="n.isDefault==1">默认地址</span>
        </p>
      </div>
...
export default {
  ...
  methods: {
    ...
    async submitOrder(){
      ...
      // 6个人参数
      let data = {
        consignee: this.userDefaultAddress.consignee,/*客户姓名*/
        consigneeTel: this.userDefaultAddress.phoneNum,/*客户号码*/
        deliveryAddress: this.userDefaultAddress.fullAddress,/*客户地址*/
        paymentWay: "ONLINE",/*支付交易方式*/
